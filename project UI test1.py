...
def ensure_deps_first_run():

    flag = Path.home() / "._tm_deps_installed"
    

    if flag.exists():
        return

    REQUIRED_PACKAGES = ["PySide6", "matplotlib", "numpy", "six>=1.16.0"]
    missing = []
    ...
    if missing:
        print(f"Первый запуск. Отсутствуют библиотеки: {', '.join(missing)}")
        print("Выполняется автоматическая установка (может занять 1-3 мин)...")
        try:
            # sys.executable гарантирует работу pip для текущего Python
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", *missing])
            flag.touch()  # Создаём метку ТОЛЬКО после успешной установки
            print("Установка завершена. Запуск приложения...")
        except Exception as e:
            print(f"Ошибка при установке: {e}")
            print("Установите вручную в терминале: pip install PySide6 matplotlib numpy")
            sys.exit(1)


ensure_deps_first_run()
...
class GraphWidget(QWidget):
    def __init__(self):
        ...
        # Создаем лейбл для картинки
        self.label = QLabel(self)
        self.label.setAlignment(Qt.AlignCenter)
        self.label.setScaledContents(True)  # Картинка растягивается под размер окна
    ...
    def update_plot(self):
        ...
        # Точка C (СМЕЩЕНА ВЛЕВО)
        Cx = 2.5
        ax.add_patch(plt.Circle((Cx, 0), 0.15, fill=False))
        ax.text(Cx, -0.5, "C")

        # Точка D (остаётся на линии)
        Dx, Dy = 3, 1.5
        ax.plot(Dx, Dy, "ko")
        ax.text(Dx - 0.3, Dy, "D")

        # Пунктир через D (НЕ через C!)
        ax.plot([Dx, Dx], [0, 3], "k--")

        # Сила P
        ax.arrow(2.5, 2.5, 0.5, -1, head_width=0.2, length_includes_head=True)
        ax.text(2.2, 2.7, "P")

        # Вертикальная сила вниз
        ax.arrow(4.5, 1, 0, -1, head_width=0.2, length_includes_head=True)
        ax.text(4.5, -0.5, "Pпл")

        # Реакции
        ax.arrow(0, 0, 0, 1.5, head_width=0.2, length_includes_head=True)
        ax.text(-0.5, 1.5, "Y0")

        ax.arrow(0, 0, 1.5, 0, head_width=0.2, length_includes_head=True)
        ax.text(1.5, -0.5, "X0")

        # Реакция в C (идёт вверх из смещённой точки)
        ax.arrow(Cx, 0, 0, 1, head_width=0.2, length_includes_head=True)
        ax.text(Cx + 0.1, 1, "Yc")

        # Угол alpha
        theta = np.linspace(0, np.arctan(3 / 6), 50)
        r = 0.8
...
class Slider(QWidget):
    def __init__(self, unit_label="", label="", min_value=0):
        super().__init__()
...
class input_panel(CastomPanel):
    def __init__(self):
        ...
        self.default_values = {"P_пл": 5, "AB": 10, "OA": 24, "OC": 8, "P": 10.4}
...
class answer_panel(QWidget):
    def __init__(self, unit_label="", label=""):
        super().__init__()
...
class SolverPage(QWidget):
    def __init__(self, app):
        ...
        main_content_layout.setContentsMargins(
            20 * scale, 20 * scale, 20 * scale, 15 * scale
        )
        main_content_layout.setSpacing(15 * scale)

        # 2. Создаем и настраиваем текст условия задачи
        task_text = (
        ...
        self.task_label.setWordWrap(True)  # Разрешаем перенос строк
        self.task_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)

        # Стиль текста
        font_task = QFont()
        font_task.setFamily("Segoe Pro")
        ...
        panels_layout.setSpacing(20 * scale)
        panels_layout.setContentsMargins(0, 0, 0, 0)

        # ---- ЛЕВАЯ ПАНЕЛЬ (Ввод и Результат) ----
        left_widget = QWidget()
        left_layout = QVBoxLayout(left_widget)
        left_layout.setContentsMargins(0, 0, 0, 0)
        left_layout.setSpacing(10 * scale)

        self.input_panel = input_panel()
        self.result_panel = result_panel(self.input_panel)

        # Устанавливаем значения из условия по умолчанию
        ...
        # Добавляем левую часть в горизонтальный лейаут
        panels_layout.addWidget(left_widget, 1)

        # ---- ПРАВАЯ ПАНЕЛЬ (График и кнопки) ----
        right_widget = QWidget()
        right_layout_inner = QVBoxLayout(right_widget)
        ...
        btns_layout.addWidget(self.hint)
        btns_layout.addWidget(self.back)
        ...
        error_layout.setAlignment(Qt.AlignBottom)

        # Объединяем низ правой панели
        bottom_right_layout = QHBoxLayout()
        bottom_right_layout.addLayout(error_layout)
        bottom_right_layout.addStretch()
        bottom_right_layout.addLayout(btns_layout)

        right_layout_inner.addLayout(bottom_right_layout)

        # Добавляем правую часть в горизонтальный лейаут
        panels_layout.addWidget(right_widget, 2)

        # Добавляем горизонтальный блок панелей под текстом
        main_content_layout.addLayout(panels_layout)
